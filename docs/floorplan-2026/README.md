...
Hover or keyboard focus reveals a booth. Click, Enter, or Space pins the popup. Another booth switches the selection. Escape, the close control, or an outside click dismisses it. The compact, medium-gray popup overlays a corner of the visible map, away from the pointer (or selected booth for keyboard/touch). It keeps a stable corner until the pointer approaches within 72px, then chooses the most distant corner available. Pinned details do not follow pointer movement. Position and size stay within the visible map area on small screens; the map's size never changes when details open. The last hovered booth remains visible while the pointer crosses the map or moves into the popup. Touch activation uses click without requiring hover; normal scrolling and browser zoom are not disabled.
